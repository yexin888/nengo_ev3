...
#define UDP_CLIENT_RX_WAIT   1    /* ms */
#define UDP_CLIENT_TX_TRIES  2

#define UDP_SERVER_RX_DELAY  10  /* us */
#define UDP_SERVER_TX_DELAY  2000000
#define UDP_SEND_POS_TIME 5000 //us
...
enum {
	...
	EV3_WRITE_FILE_COMM_OPTIM = 13,
	EV3_SEND_POS_OPTIM = 14,
};
...
char motor0_filename[100];
char motor1_filename[100];
char motor0_pos_filename[100];
char motor1_pos_filename[100];
char motor2_filename[100];
char motor2_filename2[100];
char gripper_command[]="run-to-abs-pos";
#define EOL       "\n"
...
static size_t __ev3_write_3_motors(char* data){

	send_positions();
	//printf("write 3 motors called");
	...
	int motor_count = 0;
	//motor 0 : vertical, motor 1: horizontal, motor 2: gripper
	while(value != NULL){
		FILE* f;
		//if(atoi(value)!= motor_last[motor_count]){
			if(motor_count == 0 ){
			
				f = fopen(motor0_filename,"w");
				fwrite(value,1,strlen(value)+1,f);
				fclose(f);
			}
			else if(motor_count == 1 ){
			
				f = fopen(motor1_filename,"w");
				fwrite(value,1,strlen(value)+1,f);
				fclose(f);
			}
				...
				if(atoi(value)!= motor_last[motor_count]){
					...
					f = fopen(motor2_filename,"w");
					fwrite(value,1,strlen(value)+1,f);
					fclose(f);
					f = fopen(motor2_filename2,"w");
					fwrite(gripper_command,1,strlen(gripper_command)+1,f);
					fclose(f);
				}
			}
			motor_last[motor_count] = atoi(value) ;
...
static void __ev3_find_motors(){
	...
	while((de = readdir(d))){
		...
		if(strncmp(de->d_name, "motor", 5 )==0){
		
			motor_nr = atoi(&de->d_name[5]);
			//printf("d_name: %s\n",de->d_name);
			FILE* f ;
			char filename[100];
			char addr[] = "/address";
			strcpy(filename, "/sys/class/tacho-motor/motor");
			strcat(filename, &de->d_name[5]);
			strcat(filename, addr);
			//printf("filename: %s\n",filename);
			f = fopen(filename,"r");
			if(f==NULL){
			
				ERRNO_OUT("read: fopen(%s)",filename);
				return ;
			}
			char phy_addr[20];
			fread(phy_addr, 1, 14, f );
			//printf("address: %c\n", phy_addr[13]);

			if(phy_addr[13]=='A'){
				phy_address = 0;
			}
			else if(phy_addr[13]=='B'){
				phy_address = 1;
			}
			else if(phy_addr[13]=='C'){
				phy_address = 2;
			}
			fclose(f);
			motor_map[phy_address]=motor_nr;
			printf("motor %d address %d\n", motor_nr, phy_address);
		}
	}
	char motor_name[5];
	char dest1[20];
	...
	strcpy(dest1, "/duty_cycle_sp");
	sprintf(motor_name , "%d", motor_map[0]);
	strcpy(motor0_filename, "/sys/class/tacho-motor/motor");
	strcat(motor0_filename, motor_name);
	strcat(motor0_filename, dest1 );

	strcpy(dest1, "/position");
	sprintf(motor_name , "%d", motor_map[0]);
	strcpy(motor0_pos_filename, "/sys/class/tacho-motor/motor");
	strcat(motor0_pos_filename, motor_name);
	strcat(motor0_pos_filename, dest1 );
	
	strcpy(dest1, "/duty_cycle_sp");
	sprintf(motor_name , "%d", motor_map[1]);
	strcpy(motor1_filename, "/sys/class/tacho-motor/motor");
	strcat(motor1_filename, motor_name);
	strcat(motor1_filename, dest1 );

	strcpy(dest1, "/position");
	sprintf(motor_name , "%d", motor_map[1]);
	strcpy(motor1_pos_filename, "/sys/class/tacho-motor/motor");
	strcat(motor1_pos_filename, motor_name);
	strcat(motor1_pos_filename, dest1 );

	strcpy(dest1, "/position_sp");
	strcpy(dest2, "/command");
	sprintf(motor_name , "%d", motor_map[2]);
	strcpy(motor2_filename, "/sys/class/tacho-motor/motor");
	strcat(motor2_filename, motor_name);
	strcat(motor2_filename, dest1 );
				
	strcpy(motor2_filename2, "/sys/class/tacho-motor/motor");
	strcat(motor2_filename2, motor_name);
	strcat(motor2_filename2, dest2 );
}
...
void send_positions(){
	...
	int sz = 0;
	FILE * f;
	f = fopen(motor0_pos_filename, "r");
	//getline(&buf1, &sz, f);
	fgets(buf1, 10, f);
	fclose(f);
	f = fopen(motor1_pos_filename, "r");
	//getline(&buf2, &sz, f);
	fgets(buf2, 10, f);
	fclose(f);
	//__ev3_read_binary( motor0_pos_filename, buf1, sz  );
	//__ev3_read_binary( motor1_pos_filename, buf2, sz  );
	__t_msg.head.type = EV3_SEND_POS_OPTIM; 
 	sprintf(__t_msg.body, "%d %d ", atoi(buf1),atoi(buf2) );
	__t_msg.head.data_size = strlen(__t_msg.body)+1;
	__transmit(__t_msg.head.data_size);
	//printf("send positions %s length %d\n",__t_msg.body,__t_msg.head.data_size);
...
// EV3 BRICK /////////////////////////////////////
		...
		case EV3_WRITE_FILE_COMM_OPTIM:
			//printf("calling write 3 motors...");
			//printf("data: %s\n",fn);
			__ev3_write_3_motors(fn);
			return(h->type);
...
void __ev3_udp_init(){
	...
	u_long optval = 30;
	setsockopt( sockfd, SOL_SOCKET, SO_SNDBUF, &optval, 4);
	setsockopt( sockfd, SOL_SOCKET, SO_RCVBUF, &optval , 4);
...
int main( int argc, char **argv )
{
	...
	while ( alive ) {
		...
		/*
		sleep_count ++;
		if (sleep_count == UDP_SEND_POS_TIME/UDP_SERVER_RX_DELAY){
...
char* udp_ev3_receive(){

	uint32_t received = 0;
	for (int i = 0; i < UDP_CLIENT_RX_TRIES; i++ ) {
	//while(1){
		Sleep( UDP_CLIENT_RX_WAIT );
		uint32_t result =__receive();  
		if(result== EV3_SEND_POS_OPTIM){

			received = 1;
			//return __r_msg.body;
			//return __r_msg.body;
		}
		else if (result == EV3_IDLE && received == 1){
...
int udp_ev3_write_optim(void *data, int sz)
{
	PEV3_MESSAGE_HEADER t = &__t_msg.head;
	int i;

	t->type = EV3_WRITE_FILE_COMM_OPTIM;
	t->id = ++__t_last;
	t->fn_size = 0;
	t->data_size = sz;
	//memcpy( __t_msg.body, fn, t->fn_size );
	memcpy( __t_msg.body , data, t->data_size );
	//for ( i = 0; i < UDP_CLIENT_TX_TRIES; i++ ) {
		__transmit(  t->data_size );
	//}
	return ( 0 );
